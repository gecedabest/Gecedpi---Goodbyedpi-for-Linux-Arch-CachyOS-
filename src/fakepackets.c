#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <stdint.h>
#include <sys/socket.h>
#include <netinet/in.h>
#include <netinet/ip.h>
#include <netinet/ip6.h>
#include <netinet/tcp.h>
#include <arpa/inet.h>
#include <time.h>

#include "main.h"
#include "packet.h"
#include "fakepackets.h"

static const uint8_t fake_http_request[] =
    "GET / HTTP/1.1\r\nHost: www.w3.org\r\n"
    "User-Agent: curl/7.65.3\r\nAccept: */*\r\n"
    "Accept-Encoding: deflate, gzip, br\r\n\r\n";

static const uint8_t fake_https_request[] = {
    
};

static int send_fake4(const parsed_pkt_t *pkt, uint8_t fake_ttl,
                       const uint8_t *fake_data, int fake_len)
{
    int tcp_hdr_len = pkt->tcp->doff * 4;
    int ip_hdr_len  = pkt->ip4->ihl * 4;
    int total_len   = ip_hdr_len + tcp_hdr_len + fake_len;

    if (total_len > MAX_PACKET_SIZE) {
        debug("send_fake4: packet too large (%d)\n", total_len);
        return -1;
    }

    uint8_t buf[MAX_PACKET_SIZE];
    memset(buf, 0, total_len);

    memcpy(buf, pkt->ip4, ip_hdr_len);
    struct iphdr *ip = (struct iphdr *)buf;
    ip->tot_len = htons((uint16_t)total_len);
    ip->ttl     = fake_ttl;
    ip->check   = 0;
    ip->check   = checksum_ip(ip, ip_hdr_len);

    memcpy(buf + ip_hdr_len, pkt->tcp, tcp_hdr_len);
    struct tcphdr *tcp = (struct tcphdr *)(buf + ip_hdr_len);

    memcpy(buf + ip_hdr_len + tcp_hdr_len, fake_data, fake_len);

    tcp->check = 0;
    tcp->check = checksum_tcp4(ip, tcp, buf + ip_hdr_len + tcp_hdr_len, fake_len);

    struct sockaddr_in dst;
    memset(&dst, 0, sizeof(dst));
    dst.sin_family = AF_INET;
    dst.sin_addr.s_addr = ip->daddr;

    ssize_t sent = sendto(g_rawsock4, buf, total_len, 0,
                          (struct sockaddr *)&dst, sizeof(dst));
    if (sent < 0) {
        perror("sendto(fake4)");
        return -1;
    }
    debug("send_fake4: sent %zd bytes (TTL=%d)\n", sent, fake_ttl);
    return 0;
}

int send_fake_https(const parsed_pkt_t *pkt, uint8_t fake_ttl) {
    if (!pkt->ip4 || !pkt->tcp)
        return -1;
    return send_fake4(pkt, fake_ttl,
                      fake_https_request, sizeof(fake_https_request));
}

int send_fake_http(const parsed_pkt_t *pkt, uint8_t fake_ttl) {
    if (!pkt->ip4 || !pkt->tcp)
        return -1;
    return send_fake4(pkt, fake_ttl,
                      fake_http_request, sizeof(fake_http_request) - 1);
}
